...
from typing import BinaryIO, Callable, Dict, Generator, List, Optional, Tuple, Set, Deque
...
def _classify_placement(sym: Symbol, iram_like: List[str], dram_like: List[str]) -> Optional[str]:
    ...
    if sym.is_func() and any(sec.startswith(s) for s in iram_like + ['.text', '.iram', '.iram.text', '.iram0.text', '.literal']):
        return 'IRAM_LOADER'
    # DRAM data/rodata/bss placement    
    if (sym.is_object() and any(sec.startswith(s) for s in dram_like + 
        ['.dram', '.dram0', '.dram0.bss', '.dram0.data', '.dram0.rodata', '.dram1', '.bss', '.sbss', 
        '.sdata', '.sdata2', '.srodata', '.data', '.rodata'])):
        return 'DRAM_LOADER'
    return None
...
def _origin_pattern(origin: str) -> str:
    ...
    if '(' in origin and ')' in origin:
        arch = os.path.basename(origin.split('(')[0])
        inside = origin[origin.find('(')+1:origin.rfind(')')]
        obj = os.path.basename(inside)
        stem = obj.split('.')[0]
        return f'*{arch}:{stem}.*'
...
def _symbol_name_from_map_section(section: str) -> Optional[str]:
    """Derive ELF symbol name from a linker-map input section (e.g. .sbss.mapped -> mapped)."""
    prefixes = (
        '.text.', '.literal.', '.bss.', '.sbss.', '.sbss2.',
        '.sdata.', '.sdata2.', '.srodata.', '.rodata.', '.data.',
        '.iram0.', '.iram1.',
    )
...
def _try_record_origin(
    origins: Dict[Tuple[str, int], str],
    symbol_keys: Set[Tuple[str, int]],
    name: Optional[str],
    addr: int,
    origin: str,
) -> None:
    if not name:
        return
    key = (name, addr)
    if key in symbol_keys and key not in origins:
        origins[key] = origin


def build_origin_map(map_path: str, symbol_keys: Set[Tuple[str, int]]) -> Dict[Tuple[str, int], str]:
    """
    Parse GNU ld map to associate symbols with their archive(object) origin.
    Only records origins for symbols in symbol_keys, keyed by (name, address).

    Handles multiple map line formats:
      - 0xADDR symbol_name                    (functions)
      - .section.symbol 0xADDR 0xSIZE origin  (static data, one line)
      - .section.symbol                       (static data, name on first line)
        0xADDR 0xSIZE origin                  (address/origin on second line)
    """
    origins: Dict[Tuple[str, int], str] = {}
    origin_re = re.compile(r'(?P<origin>\S+\([^\)]+\)|\S+\.(?:o|obj))\s*$')
    sym_line_re = re.compile(r'^\s*0x(?P<addr>[0-9A-Fa-f]+)\s+(?P<symbol>\S+)\s*$')
    input_section_line_re = re.compile(
        r'^\s*(?P<section>\.\S+)\s+'
        r'0x(?P<addr>[0-9A-Fa-f]+)\s+'
        r'0x(?P<size>[0-9A-Fa-f]+)\s+'
        r'(?P<origin>\S+\([^\)]+\)|\S+\.(?:o|obj))\s*$'
    )
    section_only_re = re.compile(
        r'^\s*(?P<section>\.(?:text|literal|bss|sbss|sbss2|sdata|sdata2|'
        r'srodata|rodata|data|iram0|iram1)\.\S+)\s*$'
    )
    addr_size_origin_re = re.compile(
        r'^\s*0x(?P<addr>[0-9A-Fa-f]+)\s+'
        r'0x(?P<size>[0-9A-Fa-f]+)\s+'
        r'(?P<origin>\S+\([^\)]+\)|\S+\.(?:o|obj))\s*$'
    )
    ...
    try:
        with open(map_path, 'r', encoding='utf-8', errors='ignore') as mf:
            for raw in mf:
                ...
                m_input = input_section_line_re.match(line)
                if m_input:
                    pending_section = None
                    section = m_input.group('section')
                    addr = int(m_input.group('addr'), 16)
                    origin = m_input.group('origin')
                    current_origin = origin
                    _try_record_origin(
                        origins, symbol_keys,
                        _symbol_name_from_map_section(section), addr, origin,
                    )
                    continue

                m_section = section_only_re.match(line)
                if m_section:
                    pending_section = m_section.group('section')
                    continue

                m_addr_origin = addr_size_origin_re.match(line)
                if m_addr_origin and pending_section:
                    addr = int(m_addr_origin.group('addr'), 16)
                    origin = m_addr_origin.group('origin')
                    current_origin = origin
                    _try_record_origin(
                        origins, symbol_keys,
                        _symbol_name_from_map_section(pending_section), addr, origin,
                    )
                    pending_section = None
                    continue
                ...
                if m_sym:
                    ...
                    if current_origin:
                        _try_record_origin(origins, symbol_keys, sym, addr, current_origin)
                    continue
    ...
    # Fallback: unique address in map with a single known origin
    by_addr: Dict[int, Set[str]] = {}
    for (name, addr), origin in origins.items():
        by_addr.setdefault(addr, set()).add(origin)
    for name, addr in symbol_keys:
        key = (name, addr)
        if key in origins:
            continue
        addr_origins = by_addr.get(addr)
        if addr_origins and len(addr_origins) == 1:
            origins[key] = next(iter(addr_origins))
...
def get_symbols_and_refs(rtl_list: List[str], elf_file: BinaryIO, ignore_pairs: List[IgnorePair], map_path: Optional[str] = None) -> Tuple[List[Symbol], List[Reference]]:
    ...
    if map_path:
        symbol_keys: Set[Tuple[str, int]] = {(s.name, s.addr) for s in symbols}
        origin_map = build_origin_map(map_path, symbol_keys)
        for s in symbols:
            s.origin = origin_map.get((s.name, s.addr))
...
def main() -> None:
    ...
    minset_parser = action_sub.add_parser(
        'find-reachable-from-loader-func',
        help='List the IRAM/DRAM LOADER symbols reachable from the given root functions.'
    )
    ...
    minset_parser.add_argument('--summary', action='store_true', help='Emit grouped summary by origin and show ' 
                               'the sections where the symbols should be placed')
    minset_parser.add_argument('--outplacers', action='store_true',
                               help='Print only symbols placed outside loader sections')
    ...
    symbols: List[Symbol] = []
    refs: List[Reference] = []
    if args.action in ('find-refs', 'all-refs', 'find-reachable-from-loader-func'):
    ...
    if args.action == 'find-refs':
        ...
        for r in refs:
            print(str(r))
    elif args.action == 'find-reachable-from-loader-func':
        print("Checking for reachable symbols from loader functions")
        ...
        # Default output is the full set of found symbols that are reachable from the 
        # given root functions; 
        # --outplacers limits to found symbols that are not in the loader sections
        ...
        def print_csv(items: List[Tuple[str, Symbol]]):
            print('symbol,current_section,address,type,origin,filename')
            for placement, s in sorted(items, key=sort_key_item):
                stype = 'FUNC' if s.is_func() else 'OBJECT'
                print(f'{s.name},{s.section},0x{s.addr:08x},{stype},{origin_for(s)},{s.filename or ""}')

        _LOADER_LINKER_SECTIONS: List[Tuple[str, str, str]] = [
            (
                '.iram_loader.text',
                'iram_loader_seg',
                '(.literal .text .literal.* .text.* .iram0 .iram0.* .iram1 .iram1.*)',
            ),
            (
                '.dram_loader.bss',
                'dram_loader_seg',
                '(.sbss .sbss.* .sbss2 .sbss2.* .bss .bss.*)',
            ),
            (
                '.dram_loader.data',
                'dram_loader_seg',
                '(.data .data.* .rodata .rodata.* .sdata .sdata.* .sdata2 .sdata2.* .srodata .srodata.* .dram*)',
            ),
        ]

        def print_loader_linker_summary(origins: List[str]) -> None:
            """Print linker input patterns grouped by loader output section."""
            patterns = sorted({_origin_pattern(origin) for origin in origins})
            if not patterns:
                return

            for section_name, segment_name, selectors in _LOADER_LINKER_SECTIONS:
                print(f'{section_name} :')
                print('{')
                for pattern in patterns:
                    print(f'{pattern}{selectors}')
        ...
        objs: List[Tuple[str, Symbol]] = outplacers if args.outplacers else required
        
        if args.outplacers and outplacers:
            print(f'{len(outplacers)} reachable symbols placed outside loader sections:')

        if args.summary:
            groups: Dict[Tuple[str, str, str], List[Symbol]] = {}
            for placement, s in objs:
                key = (placement, origin_for(s), s.section or '')
                groups.setdefault(key, []).append(s)
            if groups:
                print('origin,count,current_section,symbols')
                ordered = sorted(groups.items(), key=lambda x: (
                    0 if x[0][0] == 'IRAM_LOADER' else 1, x[0][1], x[0][2],
                ))
                for (placement, origin, section), items in ordered:
                    names = sorted({i.name for i in items})
                    print(f'{origin},{len(names)},{section},{";".join(names)}')
                print()
                if args.outplacers and outplacers:
                    print('Consider including the following to their respective sections in the '
                          'linker script, if not already present:\n'
                          '(Note that symbols with <unknown-origin> may need further investigation'
                          'to determine the correct origin)') 
                summary_origins = sorted({origin_for(s) for _, s in objs})
                print_loader_linker_summary(summary_origins)
        else:
            print_csv(objs)

        # Exit code: non-zero if outplacers exist when filtering to outplacers only
        raise SystemExit(1 if outplacers and args.outplacers else 0)
