...
void *esp_coex_internal_semphr_create_wrapper(uint32_t max, uint32_t init)
{
	struct k_sem *sem = (struct k_sem *)wifi_malloc(sizeof(struct k_sem));
...
void esp_coex_internal_semphr_delete_wrapper(void *semphr)
{
	esp_wifi_free(semphr);
}

int32_t esp_coex_internal_semphr_take_wrapper(void *semphr, uint32_t block_time_tick)
{
	if (block_time_tick == OSI_FUNCS_TIME_BLOCKING) {
		int ret = k_sem_take((struct k_sem *)semphr, K_FOREVER);
		if (ret == 0) {
...
int32_t esp_coex_internal_semphr_give_wrapper(void *semphr)
{
	k_sem_give((struct k_sem *)semphr);
	return 1;
}
