...
struct rtcio_context {
    unsigned int spinlock;
};

static struct rtcio_context rtcio_context = {
    .spinlock = 0
};
...
{
    ESP_RETURN_ON_FALSE(rtc_gpio_is_valid_gpio(gpio_num),
                        ESP_ERR_INVALID_ARG, RTCIO_TAG, "RTCIO number error");

    rtcio_context.spinlock = irq_lock();
    rtcio_hal_function_select(rtc_io_number_get(gpio_num), RTCIO_FUNC_RTC);
    irq_unlock(rtcio_context.spinlock);
...
esp_err_t rtc_gpio_set_drive_capability(gpio_num_t gpio_num, gpio_drive_cap_t strength)
{
    ESP_RETURN_ON_FALSE(rtc_gpio_is_valid_gpio(gpio_num),
                        ESP_ERR_INVALID_ARG, RTCIO_TAG, "RTCIO number error");
    ESP_RETURN_ON_FALSE(GPIO_IS_VALID_OUTPUT_GPIO(gpio_num),
                        ESP_ERR_INVALID_ARG, RTCIO_TAG, "Output pad only");
    ESP_RETURN_ON_FALSE(strength < GPIO_DRIVE_CAP_MAX,
                        ESP_ERR_INVALID_ARG, RTCIO_TAG, "RTCIO drive capability error");
...
esp_err_t rtc_gpio_get_drive_capability(gpio_num_t gpio_num, gpio_drive_cap_t *strength)
{
    ...
    ESP_RETURN_ON_FALSE(GPIO_IS_VALID_OUTPUT_GPIO(gpio_num),
                        ESP_ERR_INVALID_ARG, RTCIO_TAG, "Output pad only");
    ESP_RETURN_ON_FALSE(strength != NULL,
                        ESP_ERR_INVALID_ARG, RTCIO_TAG, "GPIO drive pointer error");
