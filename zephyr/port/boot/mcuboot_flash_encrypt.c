...
static esp_err_t check_and_generate_encryption_keys(void)
{
	...
	for (unsigned i = 0; i < BLOCKS_NEEDED; i++) {
		...
		if (tmp_has_key) {
			/* For ESP32: esp_efuse_find_purpose() always returns True, need to check whether the key block is used or not. */
...
esp_err_t esp_flash_encrypt_init(void)
{
	...
	/* Batch all efuse writes at the end of this function */
...
static esp_err_t encrypt_other_partition(uint32_t addr, size_t size, const char *label)
{
	...
	if (!IS_ALIGNED(addr, 32) || !IS_ALIGNED(size, 32)) {
		ESP_LOGE(TAG,
			 "Partition '%s' addr 0x%x size %u not 32-byte aligned",
			 label, addr, (unsigned int)size);
...
esp_err_t esp_flash_encrypt_contents(void)
{
	...
	DT_FOREACH_CHILD_STATUS_OKAY(DT_PARENT(DT_NODELABEL(boot_partition)),
		ENCRYPT_DT_PARTITION);
...
esp_err_t esp_flash_encrypt_enable(void)
{
	...
	defined(CONFIG_SOC_FLASH_ENCRYPTION_XTS_AES_128_DERIVED)
         ...
         * It is important to protect XTS_KEY_LENGTH_256 from further changing
		 * it to 1. Set write protection for this bit.
         * Burning WR_DIS_CRYPT_CNT, blocks further changing of eFuses:
		 * DOWNLOAD_DIS_MANUAL_ENCRYPT, SPI_BOOT_CRYPT_CNT, [XTS_KEY_LENGTH_256],
		 * SECURE_BOOT_EN.
		 */
